...
export default async function handler(req, res) {
  ...
  if (path.endsWith('/parents') && req.method === 'POST') {
  ...
  if (path.endsWith('/parents') && req.method === 'DELETE') {
    if (session.role !== 'coach' && session.role !== 'player') return res.status(403).json({ error: 'Not authorized' });
  ...
  if (req.method === 'GET') {
    ...
    const { data, error } = await supabase.from('players').select('*, parents('))').eq('season', '2026').order('squad').order('try_avg', { nullsFirst: false });
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json(data);
  }
  if (req.method === 'PATCH') {
